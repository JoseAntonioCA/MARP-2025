...
protected:
   ...
   struct TreeNode {
      ...
      TreeNode(T const& e, Link i = nullptr, Link d = nullptr,
               int alt = 1, int tam_i = 0) : elem(e), iz(i), dr(d), altura(alt), tam_i(tam_i) {}
   };
...
protected:
   ...
   int kesimoElem(Link & tree, int k) {
       if (!tree)
           return -1;
       ...
       if (tree.tam_i = k)
           ...
           if (tree.tam_i > k)
               return kesimoElem(tree.iz, k);
           else
               return kesimoElem(tree.dr, (k - tree.tam_i));
       }
   }
   ...
   void rotaIzq(Link & r1) {
      ...
      int tam_i_total = 0;
      if (r2->iz)
          tam_i_total = r1->tam_i + r2->iz->tam_i;
      else
          tam_i_total = r1->tam_i;
      r1->dr = r2->iz;
      r2->iz = r1;
      r1->altura = std::max(altura(r1->iz), altura(r1->dr)) + 1;
      r2->altura = std::max(altura(r2->iz), altura(r2->dr)) + 1;
